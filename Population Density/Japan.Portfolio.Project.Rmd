...
jp_hex <- st_read("packages/kontur_population_JP_20231101.gpkg") %>% st_transform(3106)
jp_admin <- st_read("packages/kontur_boundaries_JP_20230628.gpkg") %>% st_transform(3106)
```

```{r}
distinct_names <- jp_admin %>% distinct(name_en)
print(distinct_names)
```

```{r}
# Creating JP Boundary
jp_boundary <- jp_admin %>%
  st_geometry %>%
  st_union %>%
  st_sf %>%
  st_make_valid()
...
ggplot(jp_hex) + 
  ...
  geom_sf(data = jp_boundary, fill = NA, color = "black")
```

```{r}
# setting the ph boundary as a bounding box
bbox <- st_bbox(jp_boundary)
...
```{r}
outfile <- glue::glue("Plots/JP_final_2.1.png")
...
pop_raster <- image_read("Plots/JP_final_2.1.png")
...
pop_raster %>%
  ...
  image_write("Plots/Japan_final_annotated.plot2.png", format = "png", quality = 100)
```
