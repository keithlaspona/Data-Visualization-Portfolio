...
es_hex <- st_read("packages/kontur_population_ES_20231101.gpkg") %>% st_transform(3106)
es_admin <- st_read("packages/kontur_boundaries_ES_20230628.gpkg") %>% st_transform(3106)
```

```{r}
distinct_names <- es_admin %>% distinct(name_en)
print(distinct_names)
```

```{r}
# Creating BD Boundary
es_boundary <- es_admin %>%
  st_geometry %>%
  st_union %>%
  st_sf %>%
  st_make_valid()
...
es_boundary %>%
  ggplot()+
  geom_sf()
spn_hex <- st_intersection(es_hex, es_boundary) %>%
...
ggplot(es_hex) + 
  ...
  geom_sf(data = es_boundary, fill = NA, color = "black")
```

```{r}
# setting the ph boundary as a bounding box
bbox <- st_bbox(es_boundary)
...
```{r}
outfile <- glue::glue("Plots/Spain_final_2.png")
...
pop_raster <- image_read("Plots/Spain_final_2.png")
...
pop_raster %>%
  ...
  image_write("Plots/Annotated_plot_Spain.png", format = "png", quality = 100)
```
