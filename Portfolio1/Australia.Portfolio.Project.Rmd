...
au_hex <- st_read("packages/kontur_population_AU_20231101.gpkg") %>% st_transform(3106)
au_admin <- st_read("packages/kontur_boundaries_AU_20230628.gpkg") %>% st_transform(3106)
```

```{r}
distinct_names <- au_admin %>% distinct(name_en)
print(distinct_names)
```

```{r}
# Creating BD Boundary
au_boundary <- au_admin %>%
  st_geometry %>%
  st_union %>%
  st_sf %>%
  st_make_valid()
...
au_boundary %>%
  ggplot()+
  geom_sf()
aust_hex <- st_intersection(au_hex, au_boundary) %>%
...
ggplot(au_hex) + 
  ...
  geom_sf(data = au_boundary, fill = NA, color = "black")
```

```{r}
# setting the ph boundary as a bounding box
bbox <- st_bbox(au_boundary)
