...
is_hex <- st_read("packages/kontur_population_IS_20231101.gpkg") %>% st_transform(3106)
is_admin <- st_read("packages/kontur_boundaries_IS_20230628.gpkg") %>% st_transform(3106)
```

```{r}
distinct_names <- is_admin %>% distinct(name_en)
print(distinct_names)
```

```{r}
# Creating BD Boundary
is_boundary <- is_admin %>%
  st_geometry %>%
  st_union %>%
  st_sf %>%
  st_make_valid()
...
is_boundary %>%
  ggplot()+
  geom_sf()
iceland_hex <- st_intersection(is_hex, is_boundary) %>%
...
ggplot(is_hex) + 
  ...
  geom_sf(data = is_boundary, fill = NA, color = "black")
```

```{r}
# setting the ph boundary as a bounding box
bbox <- st_bbox(is_boundary)
...
```{r}
outfile <- glue::glue("Plots/IS_final_2.png")
