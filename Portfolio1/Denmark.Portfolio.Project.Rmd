...
dk_hex <- st_read("packages/kontur_boundaries_DK_20230628.gpkg") %>% st_transform(3106)
dk_admin <- st_read("packages/kontur_topology_boundaries_DK_20230628.gpkg") %>% st_transform(3106)
```

```{r}
distinct_names <- dk_hex %>% distinct(name_en)
print(distinct_names)
```

```{r}
# Creating BD Boundary
dk_boundary <- dk_admin %>%
  st_geometry %>%
  st_union %>%
  st_sf %>%
  st_make_valid()
...
dnmrk_hex <- st_intersection(dk_hex, dk_boundary) %>%
...
ggplot(dk_hex) + 
  ...
  geom_sf(data = dk_boundary, fill = NA, color = "black")
```

```{r}
# setting the ph boundary as a bounding box
bbox <- st_bbox(dk_boundary)
...
color <- MetBrewer::met.brewer(name="Hokusai3", direction = -1)
...
```{r}
outfile <- glue::glue("Plots/DK_final_2.png")
...
pop_raster <- image_read("Plots/NP_final_2.png")
...
pop_raster %>%
  ...
  image_write("Plots/Annotated_plot_DK.png", format = "png", quality = 100)
```
