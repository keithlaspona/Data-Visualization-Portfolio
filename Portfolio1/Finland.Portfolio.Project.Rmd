...
np_hex <- st_read("packages/kontur_population_FI_20231101.gpkg") %>% st_transform(3106)
np_admin <- st_read("packages/kontur_boundaries_FI_20230628.gpkg") %>% st_transform(3106)
```

```{r}
library(sf)

distinct_names <- np_admin %>% distinct(name_en)
...
fndlnd_hex <- st_intersection(np_hex, np_boundary) %>%
...
```{r}
outfile <- glue::glue("Plots/FI_final_2.png")
...
pop_raster <- image_read("Plots/FI_final_2.png")
