...
ggplot(gl_hex) + 
  ...
  geom_sf(data = gl_boundary, fill = NA, color = "black")
```

```{r}
# setting the ph boundary as a bounding box
bbox <- st_bbox(gl_boundary)
...
color <- MetBrewer::met.brewer(name="VanGogh3", direction = -1)
...
```{r}
outfile <- glue::glue("Plots/GreenLand_final_2.png")
...
```{r}
# Install and load the show text package
library(showtext)
library(extrafont)
library(magick)
library(colorspace)
...
pop_raster <- image_read("Plots/GreenLand_final_2.png")
...
pop_raster %>%
  ...
  image_write("Plots/Annotated_plot_gl.png", format = "png", quality = 100)
```
