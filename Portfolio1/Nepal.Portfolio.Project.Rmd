...
np_hex <- st_read("packages/kontur_boundaries_NP_20230628.gpkg") %>% st_transform(3106)
np_admin <- st_read("packages/kontur_topology_boundaries_NP_20230628.gpkg") %>% st_transform(3106)
```

```{r}
distinct_names <- np_hex %>% distinct(name_en)
print(distinct_names)
```

```{r}
# Creating BD Boundary
np_boundary <- np_admin %>%
  st_geometry %>%
  st_union %>%
  st_sf %>%
  st_make_valid()
...
islamabad_hex <- st_intersection(np_hex, np_boundary) %>%
...
ggplot(np_hex) + 
  ...
  geom_sf(data = np_boundary, fill = NA, color = "black")
```
...
color <- MetBrewer::met.brewer(name="Greek", direction = -1)
...
```{r}
outfile <- glue::glue("Plots/NP_final_2.png")
